...
while IFS= read -r IMAGE; do
    ...
    echo "=== Retagging and pushing ${IMAGE} to ${TARGET_IMAGE} ==="
    docker tag "${IMAGE}" "${TARGET_IMAGE}"
    docker push "${TARGET_IMAGE}"

    if [ -f "${TMP_DIR}/cosign.key" ]; then
        echo "=== Signing ${TARGET_IMAGE} with Cosign ==="
        cosign sign --key "${TMP_DIR}/cosign.key" "${TARGET_IMAGE}"
        echo "=== Verifying signature for ${TARGET_IMAGE} ==="
